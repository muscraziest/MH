...
/////////////////////////////////////////////////////////////////////////////////
void cruce(const vector<vector<double>> &v, vector<vector <double> > &vNormalizado, vector<vector<int>>& poblacion, vector<int> & coste_poblacion, int total_cruces, int num_filas, int num_atributos, int &num_eval){
...
// Algoritmo AGG
/////////////////////////////////////////////////////////////////////////////////
void agg(const vector<vector <double> > &v, vector<vector <double> > &vNormalizado, vector<int> &mascara, int num_filas, int num_atributos, vector<int> &resultado){

	vector<vector<int>> poblacion,poblacion_sel;
	vector<int> ev_poblacion,ev_poblacion_sel;	
	int p_cruce = 0.7;
	int num_cruces = p_cruce*30/2;
	int num_eval = 0;
	...
	generarVecindario(mascara,poblacion,30);
	...
	while(num_eval < 15000){
		...
		//Seleccionamos
		torneo(poblacion,ev_poblacion,poblacion_sel,ev_poblacion_sel,30);
...
// Algoritmo AGE
/////////////////////////////////////////////////////////////////////////////////
void age(const vector<vector <double> > &v, vector<vector <double> > &vNormalizado, vector<int> &mascara, int num_filas, int num_atributos, vector<int> &resultado){
	...
	while(num_eval < 15000){
		...
		torneo(poblacion,ev_poblacion,poblacion_sel,ev_poblacion_sel,2);

		//Cruzamos las parejas ganadoras del torneo
		cruce(v,vNormalizado,poblacion_sel,ev_poblacion_sel,1,num_filas,num_atributos,num_eval);
		
		//Mutamos
		if(Rand() <= 0.001)
			mutacion(poblacion_sel[0],1);
		if(Rand() <= 0.001)
			mutacion(poblacion_sel[1],1);

		ev_poblacion_sel[0] = funcionObjetivo(v,vNormalizado,poblacion_sel[0],num_filas,num_atributos);
		ev_poblacion_sel[1] = funcionObjetivo(v,vNormalizado,poblacion_sel[1],num_filas,num_atributos);
		num_eval+=num_filas*2;

		//Buscamos el peor de la población 
		int peor=1;
		int coste_peor = ev_poblacion[1];
		for(int i=1; i < ev_poblacion.size(); ++i){
			if(coste_peor > ev_poblacion[i]){
				coste_peor = ev_poblacion[i];
				peor = i;
			}
		}

		//Actualizamos la población
		if(ev_poblacion[peor] < ev_poblacion_sel[0]){
			ev_poblacion[peor] = ev_poblacion_sel[0];
			poblacion[peor] = poblacion_sel[0];
		}

		if(ev_poblacion[peor] < ev_poblacion_sel[1]){
			ev_poblacion[peor] = ev_poblacion_sel[1];
			poblacion[peor] = poblacion_sel[1];
		}
	}
...
int main(){
	...
	cout << "-----------------" << endl;
	cout << "Menú:" << endl;
	cout << "Opción 1: ejecutar AGG" << endl;
	cout << "Opción 2: ejecutar AGE" << endl;
	cout << "Opción 3: salir" << endl;
	cout << "-----------------" << endl;
	cin >> opcionMenu;

	while(opcionMenu != 3){// && numSimulacion <= 30){
		...
		switch(opcionMenu){
			case 1:
				cout << "Ejecutando AGG. Simulación: " << numSimulacion << endl;
				agg(mitad1,vNormalizado,resultado,filasMitad1,num_atributos,resultado);
				break;

			case 2:
				cout << "Ejecutando AGE. Simulación: " << numSimulacion << endl;
				age(mitad1,vNormalizado,resultado,filasMitad1,num_atributos,resultado);
				break;
		...
		cout << "-----------------" << endl;
		cout << "Menú:" << endl;
		cout << "Opción 1: AGG" << endl;
		cout << "Opción 2: AGE" << endl;
		cout << "Opción 3: salir" << endl;
		cout << "-----------------" << endl;
		cin >> opcionMenu;
	}
