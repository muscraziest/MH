...
// Algoritmo LS
/////////////////////////////////////////////////////////////////////////////////
void ls(const vector<vector <double> > &v, vector<vector <double> > &vNormalizado, vector<int> &mascara, int num_filas, int num_atributos, vector<int> &resultado, int & mejor_coste, int evaluaciones){
	...
	while(!fin){
		...
		for(i=0; i < vecindario.size(); ++i){
			...
			coste = funcionObjetivo(v,vNormalizado,vecindario[i],num_filas,num_atributos);
...
// Algoritmo BMB
/////////////////////////////////////////////////////////////////////////////////
void bmb(const vector<vector <double> > &v, vector<vector <double> > &vNormalizado, vector<int> &mascara, int num_filas, int num_atributos, vector<int> &resultado){

	vector<int> mejor_mascara; 
	int mejor_coste_ls;
	int mejor_coste_bmb = 0; 

	for(int i = 0; i < 25; ++i){

		mascara.clear();

		//Generamos la máscara inicial aleatoria
		generarMascaraInicial(num_atributos,mascara);

		ls(v,vNormalizado,mascara,num_filas,num_atributos,resultado,mejor_coste_ls,1000);

		if(mejor_coste_ls > mejor_coste_bmb){

			mejor_coste_bmb = mejor_coste_ls;
			mejor_mascara = resultado;
		}
	}
	
	resultado = mejor_mascara;
}
...
/////////////////////////////////////////////////////////////////////////////////
// Función del operador de mutaciónn para algoritmo ILS
/////////////////////////////////////////////////////////////////////////////////
void generarSolucionGrasp(const vector<vector <double> > &v, vector<vector <double> > &vNormalizado, vector<int> &mascara, int num_filas, int num_atributos, vector<int> &resultado){

	int n = 0.1*num_atributos;

	//Inicializamos los valores del bucle a 0
	resultado.assign(num_atributos,0);
	...
	nMejores(resultado,mejores,n);

	int indice_random = mejores[Randint(0,mejores.size()-1)];

	resultado = mascara;

	resultado[indice_random] = !resultado[indice_random];
}

/////////////////////////////////////////////////////////////////////////////////
// Algoritmo GRASP
/////////////////////////////////////////////////////////////////////////////////
void grasp(const vector<vector <double> > &v, vector<vector <double> > &vNormalizado, vector<int> &mascara, int num_filas, int num_atributos, vector<int> &resultado){


	vector <int> mascara_greedy;
	resultado = mascara;
	int coste_ls;
	int coste_grasp = 0;

	mascara.clear();
	mascara.assign(num_atributos,0);

	for(int i=0; i < 25; ++i){

		generarSolucionGrasp(v,vNormalizado,mascara,num_filas,num_atributos,mascara_greedy);

		ls(v,vNormalizado,mascara_greedy,num_filas,num_atributos,mascara,coste_ls,1000);

		if(coste_ls > coste_grasp){

			coste_grasp = coste_ls;
			resultado = mascara; 
		}
...
// Algoritmo ILS
/////////////////////////////////////////////////////////////////////////////////
void ils(const vector<vector <double> > &v, vector<vector <double> > &vNormalizado, vector<int> &mascara, int num_filas, int num_atributos, vector<int> &resultado){

	vector<int> mejor_mascara, aux; 
	int mejor_coste_ls;
	int mejor_coste_ils; 
	int t = 0.1*num_atributos;

	mascara.clear();
	mejor_mascara.clear();
	resultado.clear();

	//Generamos la mascara inicial y la guardamos como la mejor solucion hasta el momento
	generarMascaraInicial(num_atributos,mascara);
	mejor_coste_ils = funcionObjetivo(v,vNormalizado,mascara,num_filas,num_atributos);
	ls(v,vNormalizado,mascara,num_filas,num_atributos,resultado,mejor_coste_ls,1000);
	...
	for(int i = 0; i < 24; ++i){
		...
		//Optimizamos la mejor mascara
		ls(v,vNormalizado,mascara,num_filas,num_atributos,resultado,mejor_coste_ls,1000);

		//Si encontramos una mejor, actualizamos la mejor solución y mutamos
		if(mejor_coste_ls > mejor_coste_ils){

			mejor_coste_ils = mejor_coste_ls;
			mejor_mascara = resultado;
			mutacion(resultado,t);
			mascara = resultado;
		}

		//Si no, mutamos nuestra mejor solución
		else{
...
int main(){
	...
	cout << "Opción 2: BMB" << endl;
	cout << "Opción 3: GRASP:" << endl;
	cout << "Opción 4: ILS:" << endl;
	...
	while(opcionMenu != 5){
		...
		switch(opcionMenu){
			case 1:
				...
				sfs(mitad1,vNormalizado,mascara,filasMitad1,num_atributos,resultado);
				break;

			case 2:
				cout << "Ejecutando BMB. Simulación: " << numSimulacion << endl;
				bmb(mitad1,vNormalizado,mascara,filasMitad1,num_atributos,resultado);
				break;

			case 3:
				cout << "Ejecutando GRASP. Simulación: " << numSimulacion << endl;
				grasp(mitad1,vNormalizado,mascara,filasMitad1,num_atributos,resultado);
				break;

			case 4:
				cout << "Ejecutando ILS. Simulación: " << numSimulacion << endl;
				ils(mitad1,vNormalizado,mascara,filasMitad1,num_atributos,resultado);
				break;
